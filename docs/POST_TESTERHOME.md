# TestFlow - 开源AI测试平台：从Axure原型到自动化执行
...
这是TestFlow的核心优势之一。在生成测试用例时，系统会调用**Qdrant向量数据库**，自动检索四个维度的测试知识。
...
TestFlow支持以下JSON操作类型（完整的Playwright操作覆盖）：
