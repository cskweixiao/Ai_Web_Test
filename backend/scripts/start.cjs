...
async function setup() {
  try {
    // 检查 playwright 是否已安装（__dirname 是 backend/scripts 目录，所以用 ../../node_modules）
    const playwrightPath = path.resolve(__dirname, '..', '..', 'node_modules', 'playwright');
    if (!fs.existsSync(playwrightPath)) {
        console.warn('⚠️  Playwright 未安装或路径不正确');
        console.warn('💡 如需使用自动化测试功能，请运行: npm install playwright');
        console.warn('   继续启动服务...\n');
        return; // 不阻止服务启动
    }
    ...
    console.warn('⚠️  Playwright 浏览器安装失败:', error.message);
    console.warn('💡 可以稍后手动运行: npx playwright install chromium');
    console.warn('   继续启动服务...\n');
    // 不阻止服务启动
  }
}
