...
import type { TestRun, TestStep, TestLog, TestCase, TestRunStatus, TestAction } from '../../../front/src/types/test.js';
...
请只返回最匹配的元素编号（1-${elements.length}），如果没有合适的元素请返回0：`;
  ...
  private async takeStepScreenshot(runId: string, stepIndex: number | string, status: 'success' | 'failed' | 'error' | 'completed', description: string): Promise<void> {
    try {
      ...
      // 2. 使用统一的截图配置
      const { screenshotConfig } = await import('../../../front/src/utils/screenshotConfig.js');
